...
class Wsuks:
    def __init__(self, args):
        ...
        # Set args
        self.serve_only = args.serve_only
        self.targetIp = args.targetIp  # Can be None in serve-only mode
    ...
    def run(self):
        # In serve-only mode, skip MITM setup
        if self.serve_only:
            self.logger.info("===== Running in serve-only mode (no MITM) =====")
            arpspoofer = None
            router = None
        else:
